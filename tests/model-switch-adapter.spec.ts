...
describe('Grok Model Switch Image adapter', () => {
  it('registers image, maps aspectRatio, normalizes metadata, and disposes', async () => {
    ...
    const entry = root.modelSwitch.adapters.get('grok'); expect(entry).toMatchObject({ provider: 'grok', image: { provider: 'grok' } }); expect(entry).not.toHaveProperty('search'); expect(entry).not.toHaveProperty('vision')
