...
import { GROK_PROVIDER, GROK_SETTINGS_NAMESPACE } from '../src/client-contract.ts'
...
describe('settings mutate validation', () => {
  it('rejects duplicate catalog model ids and leaves saved and runtime state unchanged', async () => {
    ...
    const before = ctx.settings.describe().find(item => item.ns === GROK_SETTINGS_NAMESPACE)
    expect(before).toBeDefined()
    const ids = (await ctx.llm.listModels(GROK_PROVIDER)).map(model => model.id)
    expect(ids).toContain('kept-model')

    await expect(ctx.settings.mutate(GROK_SETTINGS_NAMESPACE, [
      { op: 'set', path: ['models'], value: [{ id: 'kept-model' }, { id: 'kept-model' }] },
    ])).rejects.toThrow(/duplicate/)

    const after = ctx.settings.describe().find(item => item.ns === GROK_SETTINGS_NAMESPACE)
    expect(after?.revision).toBe(before?.revision)
    expect(after?.value).toEqual(before?.value)
    expect(settings.writes).toHaveLength(0)
    expect((await ctx.llm.listModels(GROK_PROVIDER)).map(model => model.id)).toEqual(ids)
  })
})
