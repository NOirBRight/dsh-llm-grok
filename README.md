...
`catalogId` and the unresolved `unknown` account state are attached at runtime. The older `dsh-llm-providers-ui` 0.2.8 omits those fields and treats `unknown` as unconnected; install Owner 0.2.12 or newer.
...
With `dsh-model-switch` v0.4.5+, Grok also registers an optional Image-only adapter for the stable `generate_image` router. It reuses the same authenticated implementation and registers no Search or Vision adapter. Standalone behavior is unchanged.
...
Release and integrity: [v0.3.21](https://github.com/NOirBRight/dsh-llm-grok/releases/tag/v0.3.21) · [SHA256SUMS](https://github.com/NOirBRight/dsh-llm-grok/releases/download/v0.3.21/SHA256SUMS).
...
Validation: `pnpm test` (198 passed), `pnpm run build`; 3082 official Web selection returned real sources with both `grok-4.6` and `grok-4.5` in sequence. See the Model Switch integration audit for exact lab composition and evidence.
