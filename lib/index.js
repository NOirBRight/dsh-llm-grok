...
function decodeGrokEmptyRequest(value) {
	...
	if (!isRecord$4(value) || hasTokenFields(value)) return void 0;
	return {};
}
/**
...
function decodeGrokAuthStartReply(value) {
	if (!isRecord$4(value) || hasTokenFields(value) || typeof value["ok"] !== "boolean") return void 0;
...
function decodeGrokAuthStatus(value) {
	if (!isRecord$4(value) || hasTokenFields(value) || typeof value["loggedIn"] !== "boolean") return void 0;
...
function decodeGrokAuthLogoutReply(value) {
	if (!isRecord$4(value) || hasTokenFields(value) || value["ok"] !== true) return void 0;
	return { ok: true };
}
...
function decodeJwtPayload(token) {
	...
	try {
		...
		return isRecord$2(value) ? value : void 0;
	} catch {
		return;
	}
}
...
async function discoverOidcEndpoints(issuer, fetchImpl = fetch) {
	...
	try {
		...
		if (!isRecord$2(body)) return fallback;
...
function toolType(tool) {
	if (!isRecord$1(tool)) return void 0;
...
function injectGrokServerSearchTools(payload) {
	if (!isRecord$1(payload)) return payload;
...
//#endregion
export { Config, DEFAULT_USAGE_REQUEST_TIMEOUT_MS, GROK_AUTH_LOGOUT_ENDPOINT, GROK_AUTH_START_ENDPOINT, GROK_AUTH_STATUS_ENDPOINT, GROK_BILLING_URL, GROK_CATALOG, GROK_CHAT_BASE_URL, GROK_DEFAULT_CONTEXT_WINDOW, GROK_DEFAULT_MODEL_MAX_TOKENS, GROK_DEFAULT_STREAM_IDLE_TIMEOUT_MS, GROK_OAUTH_CLIENT_ID, GROK_OAUTH_ISSUER, GROK_OAUTH_SCOPE, GROK_PLUGIN_IDENTITY_HEADER, GROK_PROVIDER, GROK_RPC_CHANNEL, GROK_SERVER_SEARCH_TOOLS, GROK_SESSION_FILENAME, GROK_SETTINGS_NAMESPACE, GROK_USAGE_ENDPOINT, GrokAdapter, apply, createGrokAuthRuntime, createGrokPiAiProfile, createGrokRpcHandler, decodeGrokAuthLogoutReply, decodeGrokAuthStartReply, decodeGrokAuthStatus, decodeGrokEmptyRequest, decodeGrokSettings, decodeGrokUsageReply, decodeGrokUsageView, deleteSession, ensureFreshSession, grokResponsesApi, inject, injectGrokServerSearchTools, name, parseGrokBilling, readGrokUsage, readSession, refreshSession, resolveAdapterOptions, resolveGrokAccessToken, resolveGrokSessionPath, sessionPathForHome, startPkceLogin, statusFromSession, writeSession };
