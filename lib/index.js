...
var GrokSearchProvider = class {
	...
	async search(request, signal) {
		...
		if (!response.ok) {
			...
			const message = detail === void 0 ? "Grok search failed (HTTP " + String(response.status) + ")" : "Grok search failed (HTTP " + String(response.status) + "): " + detail;
