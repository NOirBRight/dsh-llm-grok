...
function decodeGrokAuthCompleteRequest(value) {
	if (!isRecord$8(value) || hasTokenFields(value)) return void 0;
...
function decodeGrokAuthStartReply(value) {
	if (!isRecord$8(value) || hasTokenFields(value) || typeof value["ok"] !== "boolean") return void 0;
...
function decodeGrokAuthStatus(value) {
	if (!isRecord$8(value) || hasTokenFields(value) || typeof value["loggedIn"] !== "boolean") return void 0;
...
function decodeGrokAuthLogoutReply(value) {
	if (!isRecord$8(value) || hasTokenFields(value) || value["ok"] !== true) return void 0;
	return { ok: true };
}
...
function decodeGrokModelsReply(value) {
	if (!isRecord$8(value) || hasTokenFields(value) || !Array.isArray(value["models"])) return void 0;
	const models = [];
...
function applyGrokReasoningWire(payload, model) {
	if (!isRecord$7(payload) || model.thinking !== true) return payload;
	const effort = resolveGrokReasoningWire((isRecord$7(payload["reasoning"]) ? payload["reasoning"] : void 0)?.["effort"], model);
	return {
		...payload,
		reasoning: { effort }
	};
...
function decodeJwtPayload(token) {
	...
	try {
		...
		return isRecord$5(value) ? value : void 0;
	} catch {
		return;
	}
}
...
async function discoverOidcEndpoints(issuer, fetchImpl = fetch) {
	...
	try {
		...
		if (!isRecord$5(body)) return fallback;
...
function toolCallFromEvent(event) {
	...
	if (!isRecord$4(block) || block["type"] !== "toolCall") return void 0;
...
function isGrokPackedReasoning(value) {
	if (!isRecord$3(value) || value["type"] !== "dsh-grok-packed-reasoning") return false;
...
function expandPackedGrokReasoningInput(payload) {
	if (!isRecord$3(payload) || !Array.isArray(payload["input"])) return payload;
	const input = [];
...
function classifyGrokTransientError(chunk) {
	...
	const code = /currently at capacity|high demand|priority processing/iu.test(message) ? "RATE_LIMIT" : /service temporarily unavailable|availability is currently degraded/iu.test(message) ? "SERVER" : void 0;
