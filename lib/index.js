...
import { INVALID_CREDENTIAL_CODE, LlmAdapter, LlmError, ReasoningEffortId, RetryPolicySchema, createUserMessage, resolveRetryPolicy } from "@deepseek-ai/dsh-llm";
...
const GROK_RPC_METHOD = "plugin-rpc/grok";
...
function decodeGrokAuthCompleteRequest(value) {
	if (!isRecord$11(value) || hasTokenFields(value)) return void 0;
...
function decodeGrokAuthStartReply(value) {
	if (!isRecord$11(value) || hasTokenFields(value) || typeof value["ok"] !== "boolean") return void 0;
	if (value["ok"] === true) {
		const attemptId = value["attemptId"];
...
function decodeGrokAuthStatus(value) {
	if (!isRecord$11(value) || hasTokenFields(value) || typeof value["loggedIn"] !== "boolean") return void 0;
...
function decodeGrokAuthLogoutReply(value) {
	if (!isRecord$11(value) || hasTokenFields(value) || value["ok"] !== true) return void 0;
	return { ok: true };
}
...
function decodeGrokModelsReply(value) {
	if (!isRecord$11(value) || hasTokenFields(value) || !Array.isArray(value["models"])) return void 0;
	const models = [];
...
function applyGrokReasoningWire(payload, model) {
	if (!isRecord$10(payload) || model.thinking !== true) return payload;
	const effort = resolveGrokReasoningWire((isRecord$10(payload["reasoning"]) ? payload["reasoning"] : void 0)?.["effort"], model);
	return {
		...payload,
		reasoning: { effort }
	};
...
async function discoverOidcEndpoints(issuer, fetchImpl = fetch) {
	...
	try {
		...
		if (!isRecord$8(body)) return fallback;
...
async function parseTokenResponse(response, now, userinfoEndpoint, fetchImpl, previous) {
	...
	if (!isRecord$8(body)) return void 0;
...
function toolCallFromEvent(event) {
	...
	if (!isRecord$7(block) || block["type"] !== "toolCall") return void 0;
...
function isGrokPackedReasoning(value) {
	if (!isRecord$6(value) || value["type"] !== "dsh-grok-packed-reasoning") return false;
...
function expandPackedGrokReasoningInput(payload) {
	if (!isRecord$6(payload) || !Array.isArray(payload["input"])) return payload;
	const input = [];
...
function toolType(tool) {
	if (!isRecord$5(tool)) return void 0;
...
*/
async function refreshGrokAccessToken(runtime) {
	const path = runtime.resolveSessionPath();
	const existing = await readSession(path);
	if (existing === void 0) throw new LlmError("llm-grok: not signed in; sign in with an xAI subscription from Plugin configuration", "MISSING_CREDENTIAL");
...
var GrokAdapter = class extends LlmAdapter {
	...
	async *stream(options) {
		yield* streamWithAuthRetry((opts) => this.current().stream(narrowGrokEscalationSchemas(opts)), options, classifyGrokTransientError, this.config.refreshApiKey);
	}
	...
	async prepareCall(provider, model, signal) {
		...
		return {
			model: inner.model,
			stream: (options) => streamWithAuthRetry((opts) => inner.stream(narrowGrokEscalationSchemas(opts)), options, classifyGrokTransientError, refreshApiKey)
		};
	}
	/**
...
function firstImage(payload, secret) {
	if (!isRecord$3(payload)) fail("Grok Imagine returned an unparseable body", secret);
	const data = payload["data"];
	if (!Array.isArray(data) || data.length === 0 || !isRecord$3(data[0])) fail("Grok Imagine returned no image data", secret);
	const row = data[0];
...
async function generateGrokImage(request) {
	...
	if (!response.ok) {
		...
		try {
			const parsed = JSON.parse(raw);
			if (isRecord$3(parsed) && isRecord$3(parsed["error"]) && typeof parsed["error"]["message"] === "string") detail = parsed["error"]["message"];
		} catch {}
...
*/
function mapGrokSearchResponse(value) {
	if (!isRecord$2(value) || !Array.isArray(value["output"])) throw new WebError("Grok returned a search response without native output", "WEB_PROVIDER_ERROR");
	const texts = [];
	const sources = [];
	const seen = /* @__PURE__ */ new Set();
	const push = (url, title, snippet) => {
		const citeable = citeableUrl(url);
		if (citeable === void 0 || seen.has(citeable)) return;
		seen.add(citeable);
		sources.push({
			url: citeable,
			...title === void 0 ? {} : { title },
	...
	for (const item of value["output"]) {
		if (!isRecord$2(item)) continue;
		const content = item["content"];
		if (Array.isArray(content)) for (const block of content) {
			if (!isRecord$2(block) || block["type"] !== "output_text") continue;
			const text = optionalString(block, "text");
			if (text !== void 0) texts.push(text);
			const annotations = block["annotations"];
			if (!Array.isArray(annotations)) continue;
			for (const annotation of annotations) {
				if (!isRecord$2(annotation) || annotation["type"] !== "url_citation") continue;
				push(annotation["url"], optionalString(annotation, "title"));
			}
		}
		const kind = item["type"];
		const results = kind === "web_search_call" || kind === "x_search_call" ? item["results"] : void 0;
		if (Array.isArray(results)) for (const result of results) {
			if (!isRecord$2(result)) continue;
			push(result["url"], optionalString(result, "title"), optionalString(result, "snippet"));
		}
	}
	if (sources.length === 0) throw new WebError("Grok returned no native search evidence; refusing to invent sources", "WEB_PROVIDER_ERROR");
...
function searchAborted(signal, fallback) {
	return new WebError("Grok search aborted", "WEB_ABORTED", { cause: signal?.aborted === true ? signal.reason : fallback });
}
...
var GrokSearchProvider = class {
	options;
	id = GROK_SEARCH_PROVIDER;
	constructor(options) {
	...
	async search(request, signal) {
		...
		if (!isSearchableGrokModel(this.options.model)) throw new WebError("unsupported Grok search model: " + this.options.model, "WEB_PROVIDER_ERROR");
		...
		if (access.length === 0) throw new WebError("Grok search is signed out; sign in from Plugin configuration", "WEB_PROVIDER_CREDENTIAL_MISSING");
		throwIfSearchAborted(signal);
		...
		try {
			...
			throwIfSearchAborted(signal);
			if (isAbortError(error)) throw searchAborted(signal, error);
			throw new WebError("Grok returned an unprocessable search response (HTTP " + String(response.status) + ")", "WEB_PROVIDER_ERROR", { cause: error });
		}
		if (!response.ok) {
...
function installGrokModelSwitchAdapters(ctx, runtime) {
	...
	const adapters = {
		...
		search: {
			provider: "grok",
			label: GROK_SEARCH_LABEL,
			get models() {
				return grokSearchModels();
			},
			supportsModel: (model) => isSearchableGrokModel(model),
	...
	ctx.inject(["modelSwitch"], (scope) => {
		...
		scope.effect(() => owner.adapters.register(adapters), "Model Switch: register Grok Search/Image adapters");
	});
}
//#endregion
...
function isZeroOrOmittedAmount(value) {
	return value === void 0 || value === null || moneyVal(value) === 0 || isRecord(value) && Object.keys(value).length === 0;
}
/**
...
*/
function hasValidCurrentPeriod(config, fetchedAt) {
...
function parseCreditsConfig(config, fetchedAt) {
	...
	if (windows.length === 0) {
		...
		if (isPercentPoints(percent)) windows.push(percentWindow(summaryId, percent, period, resetsAt));
		else if ((percent === void 0 || percent === null) && (products === void 0 || products === null || Array.isArray(products) && products.length === 0) && [
			"monthlyLimit",
			"used",
			"onDemandCap",
			"onDemandUsed",
			"prepaidBalance"
		].every((key) => isZeroOrOmittedAmount(config[key])) && hasValidCurrentPeriod(config, fetchedAt)) windows.push(percentWindow(summaryId, 0, period, resetsAt));
	}
...
async function readGrokUsage(request) {
	...
	if (response.status === 401 || response.status === 403) {
		await response.body?.cancel();
		throw new LlmError("This session cannot read Grok CLI billing. Sign out and sign in again.", INVALID_CREDENTIAL_CODE);
	}
	if (!response.ok) {
		await response.body?.cancel();
	...
	if (usage === void 0) throw new Error(redactSecrets(`${url} answered with an unrecognized billing surface`, secrets));
	return {
		status: "ok",
		usage
	};
...
function normalizeCatalogModel(model) {
	...
	const reasoningEfforts = model.reasoningEfforts?.map((effort) => {
		if (typeof effort.id !== "string" || typeof effort.value !== "string") throw new Error(`llm-grok: catalog model "${model.id}" reasoning efforts need string ids and values`);
		return {
			id: effort.id,
			value: effort.value,
...
function grokRpcResponse(rpcId, result) {
	const response = {
		type: "server-response",
		rpcId,
		result
	};
	if (!result.ok || result.attachments === void 0 || result.attachments.length === 0) return Response.json(response);
...
function createGrokRpcFetch(handler, operator) {
	return async (request) => {
		if (request.headers.get("content-type")?.split(";", 1)[0]?.trim().toLowerCase() !== "application/json") return new Response("unsupported media type", { status: 415 });
		let body;
		try {
		...
		if (message.method !== "plugin-rpc/grok") return new Response("invalid RPC method", { status: 400 });
		const value = message.payload;
		if (typeof value !== "object" || value === null || Array.isArray(value)) return new Response("invalid RPC payload", { status: 400 });
		const wrapped = value;
		if (!Object.hasOwn(wrapped, "endpoint") || typeof wrapped["endpoint"] !== "string" || Object.keys(wrapped).some((key) => key !== "endpoint" && key !== "payload")) return new Response("invalid RPC payload", { status: 400 });
		try {
...
function apply(ctx, config) {
	...
	const ensureRegistrationFacts = () => {
		...
		if (deepEqualJson(resolved.retryPolicy, registeredPolicy) && deepEqualJson(resolved.models, registeredModels)) return;
	...
	const connectionFiber = ctx.inject(["connection"], (connectionCtx) => {
		const route = {
			...
			fetch: createGrokRpcFetch(createGrokRpcHandler(runtime), connectionCtx.connection.operator)
		};
		connectionCtx.effect(() => connectionCtx.connection.fetch.register(route), "llm-grok: register authenticated Grok RPC route");
	});
	ctx.effect(() => () => connectionFiber.dispose(), "llm-grok: dispose Host Connection injection");
	ctx.inject(["settings"], (settingsCtx) => {
		settingsCtx.effect(() => settingsCtx.settings.configure({ auto: false }, ctx.fiber), "llm-grok: use custom Loader Config page");
	});
	let stopped = false;
	let imageGenFiber;
...
//#endregion
export { Config, DEFAULT_USAGE_REQUEST_TIMEOUT_MS, GROK_4_5_REASONING_EFFORTS, GROK_4_6_REASONING_EFFORTS, GROK_AUTH_ATTEMPT_STATUS_ENDPOINT, GROK_AUTH_CANCEL_ENDPOINT, GROK_AUTH_COMPLETE_ENDPOINT, GROK_AUTH_LOGOUT_ENDPOINT, GROK_AUTH_START_ENDPOINT, GROK_AUTH_STATUS_ENDPOINT, GROK_BILLING_URL, GROK_CATALOG, GROK_CHAT_BASE_URL, GROK_DEFAULT_CONTEXT_WINDOW, GROK_DEFAULT_MODEL_MAX_TOKENS, GROK_DEFAULT_REASONING_WIRE, GROK_DEFAULT_STREAM_IDLE_TIMEOUT_MS, GROK_IMAGE_GEN_TOOL_NAME, GROK_IMAGINE_ASPECT_RATIOS, GROK_IMAGINE_BASE_URL, GROK_IMAGINE_MODEL, GROK_MODELS_ENDPOINT, GROK_MODELS_URL, GROK_OAUTH_CLIENT_ID, GROK_OAUTH_ISSUER, GROK_OAUTH_SCOPE, GROK_PACKED_REASONING_TYPE, GROK_PLUGIN_IDENTITY_HEADER, GROK_PROVIDER, GROK_REASONING_WIRES, GROK_RPC_METHOD, GROK_SEARCH_LABEL, GROK_SEARCH_PROVIDER, GROK_SERVER_SEARCH_TOOLS, GROK_SESSION_FILENAME, GROK_SETTINGS_NAMESPACE, GROK_USAGE_ENDPOINT, GrokAdapter, GrokSearchProvider, apply, applyGrokReasoningWire, beginPkceLogin, cancelAllPkceLogins, cancelPkceLogin, completePkceLogin, createGrokAuthRuntime, createGrokPiAiProfile, createGrokRpcHandler, decodeGrokAuthAttemptStatus, decodeGrokAuthCompleteRequest, decodeGrokAuthLogoutReply, decodeGrokAuthStartReply, decodeGrokAuthStatus, decodeGrokEmptyRequest, decodeGrokModelsReply, decodeGrokUsageReply, decodeGrokUsageView, deleteSession, ensureFreshSession, expandPackedGrokReasoningInput, fallbackGrokCatalog, filterGrokThinkingStream, generateGrokImage, grokImageGenTool, grokResponsesApi, grokSearchModels, grokThinkingLevelMap, inject, injectGrokServerSearchTools, installGrokModelSwitchAdapters, isDisplayableThinking, isGrokPackedReasoning, isGrokServerSearchToolCallId, isSearchableGrokModel, mapGrokSearchResponse, name, officialDefaultEffort, officialEffortsFor, packGrokThinkingBlocks, parseGrokBilling, parseGrokModels, readGrokModels, readGrokUsage, readSession, refreshGrokAccessToken, refreshSession, resolveAdapterOptions, resolveGrokAccessToken, resolveGrokReasoningWire, resolveGrokSessionPath, sessionPathForHome, startPkceLogin, statusFromSession, stripGrokServerSearchToolCalls, writeSession };
