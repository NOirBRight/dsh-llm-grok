...
function installGrokModelSwitchAdapters(ctx, runtime) {
	const adapters = {
		...
		image: {
			...
			async generate(_model, request, execution) {
				if (execution === void 0) throw new Error("Grok image adapter requires public ToolRunContext");
				const value = generatedValue(await grokImageGenTool(ctx, { resolveAccessToken: () => resolveGrokAccessToken(runtime) }).execute(request, execution));
				return {
					path: value.path,
	...
	ctx.inject(["modelSwitch"], (scope) => {
		...
		scope.effect(() => owner.adapters.register(adapters), "Model Switch: register " + adapters.provider + " provider adapters");
	});
}
//#endregion
...
function createGrokRpcHandler(runtime, options) {
	return async (endpoint, payload, signal) => {
		if (endpoint === "auth/start") {
			...
			return {
				ok: true,
				value: await beginPkceLogin(runtime)
			};
		}
